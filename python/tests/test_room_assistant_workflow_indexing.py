...
async def fake_write_response(*args, **kwargs):
    return ""


async def fake_write_no_result_response(*args, **kwargs):
    return ""
...
class RoomAssistantWorkflowIndexingTests(unittest.TestCase):
    def setUp(self):
        self._patchers = [
            patch("agents.sentiment_analyzer.analyze_mood", return_value=("normal", 1.0)),
            patch("agents.response_writer.write_response", fake_write_response),
            patch("agents.response_writer.write_no_result_response", fake_write_no_result_response),
        ]
    ...
    def test_chitchat_after_search_does_not_trigger_refine_search(self):
        repo = InMemoryRoomRepository(FIXTURES)
        store = InMemorySessionStore()
        semantic = RecordingSemanticIndex()

        asyncio.run(run_room_assistant(
            "Tìm phòng dưới 7 triệu",
            session_id="s-thanks",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
    ...
    def test_budget_range_refine_and_move_in_constraints(self):
        repo = InMemoryRoomRepository(FIXTURES)
        store = InMemorySessionStore()
        semantic = RecordingSemanticIndex()

        asyncio.run(run_room_assistant(
            "Tôi muốn tìm phòng ở TP.HCM",
            session_id="s-budget-range",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ))
        result = asyncio.run(run_room_assistant(
            "Ngân sách 6-8 triệu, 1 người ở, muốn gần trung tâm, dọn trong tháng này",
            session_id="s-budget-range",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ...
        constraints = result["session_state"]["constraints"]
        self.assertEqual(result["intent"], "REFINE_SEARCH")
        self.assertEqual(constraints["budget"]["min"], 6_000_000)
        self.assertEqual(constraints["budget"]["max"], 8_000_000)
        self.assertEqual(constraints["occupants"], 1)
        self.assertEqual(constraints["move_in_date"], "this_month")
        self.assertIn("trung tam", constraints["location"]["near_landmarks"])
        self.assertEqual([room["room_id"] for room in result["rooms"]], ["B202"])

    def test_selecting_ordinal_updates_current_room_for_action_refusal(self):
        repo = InMemoryRoomRepository(FIXTURES)
        store = InMemorySessionStore()
        semantic = RecordingSemanticIndex()
        ...
        self.assertGreaterEqual(len(search["rooms"]), 2)
        expected_room_id = search["rooms"][1]["room_id"]

        detail = asyncio.run(run_room_assistant(
            "Giá phòng số 2 bao nhiêu?",
            session_id="s-select",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ...
        self.assertEqual(detail["rooms"][0]["room_id"], expected_room_id)

        selected = asyncio.run(run_room_assistant(
            "Tôi chọn phòng số 2",
            session_id="s-select",
            repository=repo,
            session_store=store,
        ...
        self.assertEqual(selected["intent"], "ASK_ABOUT_ROOM")
        self.assertEqual(selected["rooms"][0]["room_id"], expected_room_id)
        self.assertEqual(selected["session_state"]["current_room_id"], expected_room_id)

        action = asyncio.run(run_room_assistant(
            "Tôi muốn đặt phòng này",
            session_id="s-select",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ))
        self.assertEqual(action["intent"], "REQUEST_ACTION")
        self.assertEqual(action["agent_trace"]["read_tool_calls"], 0)
        self.assertEqual(action["agent_trace"]["write_tool_calls"], 0)
    ...
    def test_detail_turn_preserves_search_results_for_later_comparison(self):
        repo = InMemoryRoomRepository(FIXTURES)
        store = InMemorySessionStore()
        semantic = RecordingSemanticIndex()
        ...
        self.assertGreaterEqual(len(search["rooms"]), 3)

        detail = asyncio.run(run_room_assistant(
            "Hãy cho tôi biết giá và tiện ích của phòng đầu tiên",
            session_id="s-e2e",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ))
        self.assertEqual(detail["rooms"][0]["room_id"], search["rooms"][0]["room_id"])

        comparison = asyncio.run(run_room_assistant(
            "Hãy so sánh 3 phòng này trong một bảng",
            session_id="s-e2e",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
        ))
        self.assertEqual(comparison["intent"], "COMPARE_ROOMS")
        self.assertEqual(len(comparison["comparison"]["rows"]), 3)

        comparison = asyncio.run(run_room_assistant(
            "so sánh 3 phòng đầu tiên nhé",
            session_id="s-e2e",
            repository=repo,
            session_store=store,
            semantic_index=semantic,
    ...
    def test_search_answer_uses_result_set_template(self):
        repo = InMemoryRoomRepository(FIXTURES)
        store = InMemorySessionStore()
        semantic = RecordingSemanticIndex()

        with patch("agents.response_writer.write_response", new_callable=AsyncMock) as writer:
            writer.return_value = "Mình chỉ tìm được 1 phòng phù hợp."
            result = asyncio.run(run_room_assistant(
                "Tìm phòng dưới 7 triệu",
                session_id="s-search-template",
                repository=repo,
                session_store=store,
                semantic_index=semantic,
            ))
        ...
        self.assertGreaterEqual(len(result["rooms"]), 3)
        self.assertIn("#A101", result["answer"])
        self.assertIn("#B202", result["answer"])
        self.assertNotIn("chỉ tìm được 1", result["answer"].lower())
